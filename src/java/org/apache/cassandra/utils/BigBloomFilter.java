...
public class BigBloomFilter extends Filter
{
  ...
  BigBloomFilter(int hashes, OpenBitSet bs)
  {
    hashCount = hashes;
    bitset = bs;
  }
  ...
  /**
   * @return A BigBloomFilter with the lowest practical false positive probability
   * for the given number of elements.
   */
  public static BigBloomFilter getFilter(long numElements, int targetBucketsPerElem)
  {
      ...
      return new BigBloomFilter(spec.K, bucketsFor(numElements, spec.bucketsPerElement));
  }

  /**
   * @return The smallest BigBloomFilter that can provide the given false positive
   * probability rate for the given number of elements.
   *
   * Asserts that the given probability can be satisfied using this filter.
   */
  public static BigBloomFilter getFilter(long numElements, double maxFalsePosProbability)
  {
  ...
  {
      return BigBloomFilter.getHashBuckets(key, hashCount, buckets());
  }
  ...
  static long[] getHashBuckets(ByteBuffer b, int hashCount, long max)
  {
      ...
      long hash1 = LongMurmurHash.longHash(b.array(), 0);
      long hash2 = LongMurmurHash.longHash(b.array(), hash1);
  ...
  public void serializeBitSet(DataOutputStream dos) throws IOException
  {
